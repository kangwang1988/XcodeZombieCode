...
- (id)initWithFilePath:(NSString *)filePath {
  if ((self = [super init])) {
    ...
    if (!_dataStore) {
      [NSException raise:XCProjectNotFoundException
                  format:@"Project file not found at file path %@", _filePath];
    }

    _fileOperationQueue = [[XCFileOperationQueue alloc]
  ...
  [[self objects] enumerateKeysAndObjectsUsingBlock:^(
    ...
    if ([[obj valueForKey:@"isa"] xce_hasFileReferenceType]) {
      XcodeSourceFileType fileType = XCSourceFileTypeFromStringRepresentation(
          [obj valueForKey:@"lastKnownFileType"]
              ?: [obj valueForKey:@"explicitFileType"]);
      NSString *path = [obj valueForKey:@"path"];
      NSString *sourceTree = [obj valueForKey:@"sourceTree"];
      XCSourceFile *sourceFile = [XCSourceFile
...
- (XCSourceFile *)fileWithKey:(NSString *)key {
  NSDictionary *obj = [[self objects] valueForKey:key];
  if (obj &&
      [[obj valueForKey:@"isa"] xce_hasFileReferenceOrReferenceProxyType]) {
    XcodeSourceFileType fileType = XCSourceFileTypeFromStringRepresentation(
        [obj valueForKey:@"lastKnownFileType"]
            ?: [obj valueForKey:@"explicitFileType"]);
...
// TODO: This could fail if the path attribute on a given group is more than one
// directory. Start with candidates and
...
- (XCGroup *)createGroupWithDictionary:(NSDictionary *)dictionary
                                forKey:(NSString *)key {
  return [XCGroup
      groupWithProject:self
                   key:key
...
- (XCVersionGroup *)versionGroupWithName:(NSString *)name {
  for (XCVersionGroup *group in [self versionGroups]) {
    if ([[[group pathRelativeToParent] stringByDeletingPathExtension]
...
- (XCVersionGroup *)createVersionGroupWithDictionary:(NSDictionary *)dictionary
                                              forKey:(NSString *)key {
  return [XCVersionGroup
      versionGroupWithProject:self
                          key:key
                         path:[dictionary valueForKey:@"path"]
                     children:[dictionary valueForKey:@"children"]
...
- (NSArray *)applicationTargets {
  ...
  NSArray *filteredTargets = [targets
...
- (void)save {
  ...
  NSData *data = [NSPropertyListSerialization
      dataWithPropertyList:_dataStore
                    format:NSPropertyListXMLFormat_v1_0
                   options:0
                     error:NULL];
  data = [self _fixEncodingInData:data];
  [data
      writeToFile:[_filePath stringByAppendingPathComponent:@"project.pbxproj"]
       atomically:YES];
...
- (NSDictionary *)configurations {
  if (_configurations == nil) {
    NSString *buildConfigurationRootSectionKey =
        [[[self objects] objectForKey:[self rootObjectKey]]
            objectForKey:@"buildConfigurationList"];
