...
// Goes to the entry for this object in the project and sets a value for one of
// the keys, such as name, path, etc.
- (void)setValue:(id)val forProjectItemPropertyWithKey:(NSString *)key {
  NSMutableDictionary *obj =
      [[[_project objects] objectForKey:_key] mutableCopy];
  if (nil == obj) {
    [NSException raise:@"Project item not found"
                format:@"Project item with key %@ not found.", _key];
  }
...
- (BOOL)canBecomeBuildFile {
  return _type == SourceCodeObjC || _type == SourceCodeObjCPlusPlus ||
         ...
         _type == Framework || _type == ImageResourcePNG || _type == HTML ||
         _type == Bundle || _type == Archive || _type == AssetCatalog ||
         _type == SourceCodeSwift || _type == PropertyList ||
         _type == LocalizableStrings;
}

- (XcodeMemberType)buildPhase {
  if (_type == SourceCodeObjC || _type == SourceCodeObjCPlusPlus ||
      _type == SourceCodeCPlusPlus || _type == XibFile ||
    ...
    return PBXFrameworksBuildPhaseType;
             ...
             _type == AssetCatalog || _type == PropertyList ||
             _type == LocalizableStrings) {
  ...
  if (![self isBuildFile]) {
    if ([self canBecomeBuildFile]) {
      ...
      NSString *buildFileKey = [[XCKeyBuilder
          forItemNamed:[_name stringByAppendingString:@".buildFile"]] build];
      [_project objects][buildFileKey] = sourceBuildFile;
    } else if (_type == Framework) {
      [NSException raise:NSInvalidArgumentException
                  format:@"Add framework to target not implemented yet."];
    } else {
      [NSException raise:NSInvalidArgumentException
                  format:@"Project file of type %@ can't become a build file.",
...
- (void)setCompilerFlags:(NSString *)value {
  ...
  [objectArrayCopy enumerateKeysAndObjectsUsingBlock:^(
                       NSString *key, NSDictionary *obj, BOOL *stop) {
    if ([[obj valueForKey:@"isa"] xce_hasBuildFileType]) {
      ...
      if ([obj[@"fileRef"] isEqualToString:self.key]) {
        ...
        if (![replaceBuildFile objectForKey:@"settings"]) {
          ...
          if (![settingsDict objectForKey:@"ATTRIBUTES"]) {
            ...
            NSMutableArray *attr =
                [NSMutableArray arrayWithArray:settingsDict[@"ATTRIBUTES"]];
...
- (NSString *)pathRelativeToProjectRoot {
  NSString *parentPath =
      [[_project groupForGroupMemberWithKey:_key] pathRelativeToProjectRoot];
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"Project file: key=%@, name=%@, fullPath=%@",
                                 _key, _name, [self pathRelativeToProjectRoot]];
}
