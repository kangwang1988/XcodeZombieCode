...
+ (XCTarget *)targetWithProject:(XCProject *)project
  ...
  return [[XCTarget alloc] initWithProject:project
                                       key:key
                                      name:name
...
- (NSArray *)resources {
  if (_resources == nil) {
    ...
    for (NSString *buildPhaseKey in
         [[[_project objects] objectForKey:_key] objectForKey:@"buildPhases"]) {
      NSDictionary *buildPhase =
          [[_project objects] objectForKey:buildPhaseKey];
...
- (NSDictionary *)configurations {
  if (_configurations == nil) {
    NSString *buildConfigurationRootSectionKey = [[[_project objects]
        objectForKey:_key] objectForKey:@"buildConfigurationList"];
    NSDictionary *buildConfigurationDictionary =
        [[_project objects] objectForKey:buildConfigurationRootSectionKey];
    _configurations = [[XCProjectBuildConfig
        buildConfigurationsFromArray:[buildConfigurationDictionary
                                         objectForKey:@"buildConfigurations"]
                           inProject:_project] mutableCopy];
    _defaultConfigurationName = [[buildConfigurationDictionary
        objectForKey:@"defaultConfigurationName"] copy];
  }
...
- (NSArray *)members {
  if (_members == nil) {
    ...
    for (NSString *buildPhaseKey in
      ...
      NSDictionary *buildPhase =
          [[_project objects] objectForKey:buildPhaseKey];
      if ([[buildPhase valueForKey:@"isa"]
              xce_hasSourcesOrFrameworksBuildPhaseType]) {
        for (NSString *buildFileKey in [buildPhase objectForKey:@"files"]) {
          id<XCBuildFile, XcodeGroupMember> targetMember =
              [self buildFileWithKey:buildFileKey];
          if (targetMember) {
  ...
  if (_buildShellScripts == nil) {
    ...
    for (NSString *buildPhaseKey in
      ...
      if ([[buildPhase valueForKey:@"isa"] xce_hasShellScriptBuildPhase]) {
        ...
        XCBuildShellScript *targetMember = [[XCBuildShellScript alloc]
            ...
            runOnlyForDeploymentPostprocessing:
                [buildPhase[@"runOnlyForDeploymentPostprocessing"] boolValue]
...
- (void)addMember:(id<XCBuildFile>)member {
  ...
  for (NSString *buildPhaseKey in [target objectForKey:@"buildPhases"]) {
    NSMutableDictionary *buildPhase =
        [[_project objects] objectForKey:buildPhaseKey];
    if ([[buildPhase valueForKey:@"isa"] xce_asMemberType] ==
        [member buildPhase]) {
...
- (void)makeAndAddShellScript:(XCBuildShellScriptDefinition *)shellScript {
  ...
  NSDictionary *reference = @{
    ...
    @"runOnlyForDeploymentPostprocessing" :
        @(shellScript.runOnlyForDeploymentPostprocessing),
...
- (NSDictionary *)buildRefWithFileRefKey {
  NSMutableDictionary *buildRefWithFileRefDict =
...
- (instancetype)duplicateWithTargetName:(NSString *)targetName
  ...
  void (^visitor)(NSMutableDictionary *) =
      ^(NSMutableDictionary *buildConfiguration) {
        buildConfiguration[@"buildSettings"][@"PRODUCT_NAME"] = productName;
      };

  dupTargetObj[@"buildConfigurationList"] = [XCProjectBuildConfig
      duplicatedBuildConfigurationListWithKey:buildConfigurationListKey
                                    inProject:_project
  ...
  return [[XCTarget alloc] initWithProject:_project
                          ...
                          productReference:dupTargetObj[@"productReference"]
                               productType:dupTargetObj[@"productType"]];
}
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"Target: name=%@, files=%@", _name, _members];
}
...
- (id<XCBuildFile, XcodeGroupMember>)buildFileWithKey:(NSString *)theKey {
  ...
  if (obj) {
    if ([[obj valueForKey:@"isa"] xce_hasBuildFileType]) {
      id<XcodeGroupMember> targetFile =
          [_project groupMemberWithKey:[obj valueForKey:@"fileRef"]];
...
- (void)duplicateProductReferenceForTargetObject:
            (NSMutableDictionary *)dupTargetObj
                                 withProductName:(NSString *)productName {

  NSString *productReferenceKey = dupTargetObj[@"productReference"];
  NSMutableDictionary *dupProductReference =
      [_project.objects[productReferenceKey] mutableCopy];
...
- (void)duplicateBuildPhasesForTargetObject:
    (NSMutableDictionary *)dupTargetObj {
  ...
  for (NSString *buildPhaseKey in dupTargetObj[@"buildPhases"]) {
    ...
    NSMutableDictionary *dupBuildPhase =
        [_project.objects[buildPhaseKey] mutableCopy];
...
- (void)addReferenceToProductsGroupForTargetObject:
    (NSMutableDictionary *)dupTargetObj {

  XCGroup *mainGroup = nil;
  NSPredicate *productsPredicate =
      [NSPredicate predicateWithFormat:@"displayName == 'Products'"];
  NSArray *filteredGroups =
      [_project.groups filteredArrayUsingPredicate:productsPredicate];

  if (filteredGroups.count > 0) {
    mainGroup = filteredGroups[0];
    NSMutableArray *children =
        [_project.objects[mainGroup.key][@"children"] mutableCopy];
