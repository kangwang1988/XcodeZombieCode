...
+ (void)load {
  VariationSelectors =
      [NSCharacterSet characterSetWithRange:NSMakeRange(0xFE00, 16)];
}

- (BOOL)isEmoji {
  if ([self rangeOfCharacterFromSet:VariationSelectors].location !=
      NSNotFound) {
    return YES;
  }
...
- (instancetype)stringByRemovingEmoji {
  NSMutableString *__block buffer =
  ...
  [self
      enumerateSubstringsInRange:NSMakeRange(0, [self length])
                      ...
                      usingBlock:^(NSString *substring, NSRange substringRange,
                                   NSRange enclosingRange, BOOL *stop) {
                        [buffer appendString:([substring isEmoji]) ? @""
                                                                   : substring];
                      }];
