...
// returns the key for the reference proxy with the given path (nil if not
// found)
// does not use keysForProjectObjectsOfType:withIdentifier: because the
// identifier it uses for
// PBXReferenceProxy is different.
- (NSString *)referenceProxyKeyForName:(NSString *)name {
  __block NSString *result = nil;
  [[self objects] enumerateKeysAndObjectsUsingBlock:^(
...
// returns an array of build products, excluding bundles with extensions other
// than ".bundle" (which is kind
// of gross, but I didn't see a better way to exclude test bundles without
// giving them their own XcodeSourceFileType)
  ...
  [[self objects] enumerateKeysAndObjectsUsingBlock:^(
    ...
    if ([[obj valueForKey:@"isa"] xce_hasReferenceProxyType]) {
      ...
      NSString *containerPortal =
          [containerProxy valueForKey:@"containerPortal"];
      if ([containerPortal isEqualToString:xcodeprojKey]) {
        XcodeSourceFileType type = XCSourceFileTypeFromStringRepresentation(
            [obj valueForKey:@"fileType"]);
        NSString *path = (NSString *)[obj valueForKey:@"path"];
        if (type != Bundle ||
            [[path pathExtension] isEqualToString:@"bundle"]) {
...
// makes PBXContainerItemProxy and PBXTargetDependency objects for the
// xcodeproj, and adds the dependency key
// to all the specified targets
- (void)addAsTargetDependency:(XCSubProjectDefinition *)xcodeprojDefinition
                    toTargets:(NSArray *)targets {
  for (XCTarget *target in targets) {
    // make a new PBXContainerItemProxy
    NSString *key = [[self
        fileWithName:[xcodeprojDefinition pathRelativeToProjectRoot]] key];
    NSString *containerItemProxyKey =
        [self makeContainerItemProxyForName:[xcodeprojDefinition name]
                                    fileRef:key
                                  proxyType:@"1"
                                 uniqueName:[target name]];
    // make a PBXTargetDependency
    NSString *targetDependencyKey =
        [self makeTargetDependency:[xcodeprojDefinition name]
...
// returns an array of keys for all project objects (not just files) that match
// the given criteria.  Since this is
// a convenience method intended to save typing elsewhere, each type has its own
// field to match to rather than each
...
- (NSArray *)keysForProjectObjectsOfType:(XcodeMemberType)memberType
  ...
  [[self objects] enumerateKeysAndObjectsUsingBlock:^(
    ...
    if ([[obj valueForKey:@"isa"] xce_asMemberType] == memberType) {
      ...
      } else if (memberType == PBXTargetDependencyType ||
                 memberType == PBXGroupType ||
                 memberType == PBXVariantGroupType) {
      ...
      } else if (memberType == PBXFrameworksBuildPhaseType ||
                 memberType == PBXResourcesBuildPhaseType) {
        [returnValue addObject:key];
      } else {
        [NSException raise:NSInvalidArgumentException
                    format:@"Unrecognized member type %@",
                           [NSString xce_stringFromMemberType:memberType]];
      }
    }
  }];
  if (singleton && [returnValue count] > 1) {
    [NSException raise:NSGenericException
                format:@"Searched for one instance of member type %@ with "
                       @"value %@, but found %ld",
                       [NSString xce_stringFromMemberType:memberType],
                       identifier, (unsigned long)[returnValue count]];
  }
  if (required && [returnValue count] == 0) {
    [NSException
         raise:NSGenericException
        format:@"Searched for instances of member type %@ with value %@, but "
               @"did not find any",
...
// returns the dictionary for the PBXProject.  Raises an exception if more or
// less than 1 are found.
- (NSMutableDictionary *)PBXProjectDict {
  NSString *PBXProjectKey;
  NSArray *PBXProjectKeys = [self keysForProjectObjectsOfType:PBXProjectType
                                               withIdentifier:nil
                                                    singleton:YES
                                                     required:YES];
  PBXProjectKey = [PBXProjectKeys objectAtIndex:0];
  NSMutableDictionary *PBXProjectDict =
      [[self objects] valueForKey:PBXProjectKey];
  return PBXProjectDict;
}

// returns the key of the PBXContainerItemProxy for the given name and proxy
// type. nil if not found.
- (NSString *)containerItemProxyKeyForName:(NSString *)name
  ...
  [[self objects] enumerateKeysAndObjectsUsingBlock:^(
    ...
    if ([[obj valueForKey:@"isa"] xce_hasContainerItemProxyType]) {
      ...
      if ([remoteInfo isEqualToString:name] &&
          [proxy isEqualToString:proxyType]) {
  ...
  if ([results count] > 1) {
    [NSException raise:NSGenericException
                format:@"Searched for one instance of member type %@ with "
                       ...
                       [NSString stringWithFormat:@"%@ and proxyType of %@",
                                                  name, proxyType],
...
// makes a PBXContainerItemProxy object for a given PBXFileReference object.
...
- (NSString *)makeContainerItemProxyForName:(NSString *)name
  ...
  // give it a random key - the keys xcode puts here are not in the project file
  // anywhere else
  NSString *key = [[XCKeyBuilder
      forItemNamed:[NSString stringWithFormat:@"%@-junk", keyName]] build];
  proxy[@"remoteGlobalIDString"] = key;
  proxy[@"remoteInfo"] = name;
  // add to project. use proxyType to generate key, so that multiple keys for
  // the same name don't overwrite each other
  key = [[XCKeyBuilder
      forItemNamed:[NSString stringWithFormat:@"%@-containerProxy-%@", keyName,
                                              proxyType]] build];
...
- (void)makeReferenceProxyForContainerItemProxy:
  ...
  NSArray *existingProxyKeys =
      [self keysForProjectObjectsOfType:PBXReferenceProxyType
                         withIdentifier:path
                              singleton:NO
                               required:NO];
  ...
  // add to project
  NSString *key = [[XCKeyBuilder
      forItemNamed:[NSString stringWithFormat:@"%@-referenceProxy", path]]
      build];
  [self objects][key] = proxy;
}

// makes a PBXTargetDependency object for a given PBXContainerItemProxy.
// Replaces pre-existing objects.
- (NSString *)makeTargetDependency:(NSString *)name
  ...
  NSArray *existingDependencyKeys =
      [self keysForProjectObjectsOfType:PBXTargetDependencyType
  ...
  targetDependency[@"isa"] =
      [NSString xce_stringFromMemberType:PBXTargetDependencyType];
  targetDependency[@"name"] = name;
  targetDependency[@"targetProxy"] = containerItemProxyKey;
  NSString *targetDependencyKey = [[XCKeyBuilder
      forItemNamed:[NSString stringWithFormat:@"%@-targetProxy", keyName]]
      build];
  [self objects][targetDependencyKey] = targetDependency;
  return targetDependencyKey;
}

// make a PBXContainerItemProxy and PBXReferenceProxy for each target in the
// subProject
- (void)addProxies:(XCSubProjectDefinition *)xcodeproj {
  NSString *fileRef =
      [[self fileWithName:[xcodeproj pathRelativeToProjectRoot]] key];
  for (NSDictionary *target in [xcodeproj.subProject targets]) {
    NSString *containerItemProxyKey =
        [self makeContainerItemProxyForName:[target valueForKey:@"name"]
                                    fileRef:fileRef
                                  proxyType:@"2"
                                 uniqueName:nil];
    NSString *productFileReferenceKey =
        [target valueForKey:@"productReference"];
    NSDictionary *productFileReference =
        [[xcodeproj.subProject objects] valueForKey:productFileReferenceKey];
    [self makeReferenceProxyForContainerItemProxy:containerItemProxyKey
                            buildProductReference:productFileReference];
  }
}

// remove the PBXContainerItemProxy and PBXReferenceProxy objects for the given
// object key (which is the PBXFilereference
// for the xcodeproj file)
- (void)removeProxies:(NSString *)xcodeprojKey {
  NSMutableArray *keysToDelete = [NSMutableArray array];
  // use the xcodeproj's PBXFileReference key to get the PBXContainerItemProxy
  // keys
  NSArray *containerItemProxyKeys =
      [self keysForProjectObjectsOfType:PBXContainerItemProxyType
  ...
  for (NSString *key in containerItemProxyKeys) {
    [keysToDelete addObjectsFromArray:
                      [self keysForProjectObjectsOfType:PBXReferenceProxyType
                                         withIdentifier:key
                                              singleton:NO
                                               required:NO]];
...
// returns the Products _group key for the given PBXFileReference key, nil if
// not found.
- (NSString *)productsGroupKeyForKey:(NSString *)key {
  NSMutableArray *projectReferences =
      [[self PBXProjectDict] valueForKey:@"projectReferences"];
  ...
  for (NSDictionary *projectRef in projectReferences) {
    if ([[projectRef valueForKey:@"ProjectRef"] isEqualToString:key]) {
      ...
      if (productsGroupKey != nil) {
        [NSException
             raise:NSGenericException
            format:@"Found more than one project reference for key %@", key];
      }
...
// removes a file reference from the projectReferences array in PBXProject
// (removing the array itself if this action
// leaves it empty).
- (void)removeFromProjectReferences:(NSString *)key
                   forProductsGroup:(NSString *)productsGroupKey {
  NSMutableArray *projectReferences =
      [[self PBXProjectDict] valueForKey:@"projectReferences"];
...
// removes a specific xcodeproj file from any targets (by name).  It's not an
// error if no entries are found,
// because we support adding a project file without adding it to any targets.
- (void)removeTargetDependencies:(NSString *)name {
  // get the key for the PBXTargetDependency with name = xcodeproj file name
  // (without extension)
  NSArray *targetDependencyKeys =
      [self keysForProjectObjectsOfType:PBXTargetDependencyType
                         withIdentifier:name
                              singleton:NO
                               required:NO];
  // we might not find any if the project wasn't added to targets in the first
  // place
  if ([targetDependencyKeys count] == 0) {
    return;
  }
  NSString *targetDependencyKey = targetDependencyKeys[0];
  // use the key for the PBXTargetDependency to get the key for any
  // PBXNativeTargets that depend on it
  NSArray *nativeTargetKeys =
      [self keysForProjectObjectsOfType:PBXNativeTargetType
                         withIdentifier:targetDependencyKey
                              singleton:NO
                               required:NO];
  // remove the key for the PBXTargetDependency from the PBXNativeTarget's
  // dependencies arrays (leave in place even if empty)
