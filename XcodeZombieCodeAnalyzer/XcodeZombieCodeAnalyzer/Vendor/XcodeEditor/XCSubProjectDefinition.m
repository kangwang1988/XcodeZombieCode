...
+ (XCSubProjectDefinition *)withName:(NSString *)name
  ...
  return [[XCSubProjectDefinition alloc] initWithName:name
                                                 path:path
...
// Note - _path is most often going to be an absolute path.  The method
// pathRelativeToProjectRoot below should be
...
- (id)initWithName:(NSString *)name
  ...
  if (self) {
    ...
    _subProject = [[XCProject alloc]
        initWithFilePath:[NSString
                             stringWithFormat:@"%@/%@.xcodeproj", path, name]];
  }
  return self;
}
...
- (NSString *)fullPathName {
  return
      [NSString stringWithFormat:@"%@/%@", _path,
                                 [_name stringByAppendingString:@".xcodeproj"]];
}
...
- (NSArray *)buildProductNames {
  ...
  [objects enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSDictionary *obj,
                                               BOOL *stop) {
...
- (NSString *)projectKey {
  if (_key == nil) {
    NSArray *xcodeprojKeys = [_parentProject
        keysForProjectObjectsOfType:PBXFileReferenceType
                     withIdentifier:[self pathRelativeToProjectRoot]
                          singleton:YES
                           required:YES];
...
- (void)initFullProjectPath:(NSString *)fullProjectPath
                  groupPath:(NSString *)groupPath {
  if (groupPath != nil) {
    NSMutableArray *fullPathComponents =
        [[fullProjectPath pathComponents] mutableCopy];
    [fullPathComponents removeLastObject];
    fullProjectPath = [[NSString pathWithComponents:fullPathComponents]
        stringByAppendingFormat:@"/%@", groupPath];
  }
  _fullProjectPath = [fullProjectPath copy];
}

// compares the given path to the filePath of the project, and returns a
// relative version. _fullProjectPath, which has
// to hve been previously set, is the full path to the project *plus* the path
// to the xcodeproj's _group, if any.
- (NSString *)pathRelativeToProjectRoot {
  if (_relativePath == nil) {
    if (_fullProjectPath == nil) {
      [NSException raise:NSInvalidArgumentException
                  format:@"fullProjectPath has not been set"];
    }
    NSMutableArray *projectPathComponents =
        [[_fullProjectPath pathComponents] mutableCopy];
    ...
    NSInteger limit =
        ([projectPathComponents count] < [objectPathComponents count])
            ? [projectPathComponents count]
    ...
    // insert "../" for each remaining path component in project's xcodeproj
    // path
    for (NSInteger index2 = 0;
         index2 < ([projectPathComponents count] - index1); index2++) {
      convertedPath = [convertedPath stringByAppendingString:@"../"];
    }
    // tack on the unique part of the object's path
    for (NSInteger index3 = index1; index3 < [objectPathComponents count] - 1;
         index3++) {
      convertedPath = [convertedPath
          stringByAppendingFormat:@"%@/", objectPathComponents[index3]];
    }
    _relativePath = [[convertedPath
        stringByAppendingString:[objectPathComponents lastObject]] copy];
  }
...
- (NSString *)description {
  return [NSString
      stringWithFormat:
          @"XcodeprojDefinition: sourceFileName = %@, path=%@, type=%li", _name,
          _path, (long)_type];
}
