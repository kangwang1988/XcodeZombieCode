...
- (NSString *)destinationPathFor:(NSString *)fileName
...
- (void)queueTextFile:(NSString *)fileName
  ...
  [_filesToWrite setObject:[contents dataUsingEncoding:NSUTF8StringEncoding]
                    forKey:[self destinationPathFor:fileName
                                 inProjectDirectory:directory]];
}
...
- (void)queueDataFile:(NSString *)fileName
  ...
  [_filesToWrite setObject:contents
                    forKey:[self destinationPathFor:fileName
                                 inProjectDirectory:directory]];
}
...
- (void)queueDirectory:(NSString *)withName
  ...
  [_directoriesToCreate addObject:[self destinationPathFor:withName
                                        inProjectDirectory:parentDirectory]];
}
...
- (NSString *)destinationPathFor:(NSString *)fileName
              inProjectDirectory:(NSString *)directory {
  return [[_baseDirectory stringByAppendingPathComponent:directory]
      stringByAppendingPathComponent:fileName];
}

- (void)performFileWrites {
  [_filesToWrite enumerateKeysAndObjectsUsingBlock:^(NSString *filePath,
                                                     NSData *data, BOOL *stop) {
    NSError *error = nil;
    if (![data writeToFile:filePath options:NSDataWritingAtomic error:&error]) {
      [NSException raise:NSInternalInconsistencyException
                  format:@"Error writing file at filePath: %@, error: %@",
                         filePath, error];
    }
  }];
...
- (void)performCopyFrameworks {
  [_frameworksToCopy
      enumerateKeysAndObjectsUsingBlock:^(NSURL *destinationUrl,
        ...
        NSError *error = nil;
        if (![fileManager copyItemAtURL:frameworkPath
...
- (void)performFileDeletions {
  for (NSString *filePath in [_filesToDelete reverseObjectEnumerator]) {
    NSString *fullPath =
        [_baseDirectory stringByAppendingPathComponent:filePath];
    NSError *error = nil;
    ...
    if (![fileManager fileExistsAtPath:filePath]) {
      if (![fileManager createDirectoryAtPath:filePath
        ...
        [NSException raise:NSInvalidArgumentException
                    format:@"Error: Create folder failed %@", filePath];
      }
    }
  }
}
