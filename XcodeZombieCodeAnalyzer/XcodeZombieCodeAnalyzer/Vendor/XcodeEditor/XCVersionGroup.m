...
+ (XCVersionGroup *)versionGroupWithProject:(XCProject *)project
  ...
  return [[XCVersionGroup alloc] initWithProject:project
                                             key:key
                                            path:path
                                        children:children
...
- (void)addDataModelSource:(XCSourceFileDefinition *)sourceFileDefinition {
  if ([sourceFileDefinition type] == XCDataModel) {
    ...
    [NSException
        ...
        format:@"Project file of type %@ can't be a child of a %@",
               _versionGroupType,
               NSStringFromXCSourceFileType([sourceFileDefinition type])];
  }
}
...
// creates PBXFileReference and adds to group if not already there;  returns key
// for file reference.  Locates
