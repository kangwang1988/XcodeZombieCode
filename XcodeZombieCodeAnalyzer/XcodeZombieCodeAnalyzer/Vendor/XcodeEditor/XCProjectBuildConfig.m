...
+ (NSDictionary *)buildConfigurationsFromArray:(NSArray *)array
  ...
  for (NSString *buildConfigurationKey in array) {
    NSDictionary *buildConfiguration =
        [[project objects] objectForKey:buildConfigurationKey];

    if ([[buildConfiguration valueForKey:@"isa"]
            xce_hasBuildConfigurationType]) {
      XCProjectBuildConfig *configuration = [configurations
          objectForKey:[buildConfiguration objectForKey:@"name"]];
      if (!configuration) {
        configuration = [[XCProjectBuildConfig alloc]
            initWithProject:project
                        key:buildConfigurationKey];

        [configurations setObject:configuration
                           forKey:[buildConfiguration objectForKey:@"name"]];
      }

      XCSourceFile *configurationFile =
          [project fileWithKey:[buildConfiguration
                                   objectForKey:@"baseConfigurationReference"]];
      if (configurationFile) {
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
          ...
          do {
            path = [[group pathRelativeToParent]
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
          path = [[[project filePath] stringByDeletingLastPathComponent]
              stringByAppendingPathComponent:path];
        }

        if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
          path = [[[project filePath] stringByDeletingLastPathComponent]
              stringByAppendingPathComponent:configurationFile.path];
        }

        if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
          NSLog(@"XCConfig not found. Unable to find XCConfig file at %@",
                path);
        }
      }

      [configuration
          addBuildSettings:[buildConfiguration objectForKey:@"buildSettings"]];
    }
  }
...
+ (NSString *)duplicatedBuildConfigurationListWithKey:
  ...
  NSMutableDictionary *dupBuildConfigurationList =
      [buildConfigurationList mutableCopy];

  NSMutableArray *dupBuildConfigurations = [NSMutableArray array];

  for (NSString *buildConfigurationKey in
           buildConfigurationList[@"buildConfigurations"]) {
    [dupBuildConfigurations
        addObject:
            [self
                duplicatedBuildConfigurationWithKey:buildConfigurationKey
                                          inProject:project
...
- (void)addOrReplaceSetting:(id<NSCopying>)setting forKey:(NSString *)key {
  ...
  NSMutableDictionary *dict =
      [[[_project objects] objectForKey:_key] mutableCopy];
...
- (NSString *)description {
  ...
  [description appendFormat:@"build settings: %@, inherited: %@",
                            _buildSettings, _xcconfigSettings];
