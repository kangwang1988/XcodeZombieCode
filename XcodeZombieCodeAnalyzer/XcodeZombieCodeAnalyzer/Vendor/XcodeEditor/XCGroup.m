...
- (void)addClass:(XCClassDefinition *)classDefinition
  ...
  XCSourceFile *sourceFile =
      [_project fileWithName:[classDefinition sourceFileName]];
  [self addSourceFile:sourceFile toTargets:targets];
}

- (void)removeClass:(XCClassDefinition *)classDefinition
        fromTargets:(NSArray *)targets {
  XCSourceFile *sourceFile =
      [_project fileWithName:[classDefinition className]];
  [sourceFile removeBuildFile];

  NSString *classKey = sourceFile.key;
  // XCSourceFile *frameworkSourceRef = (XCSourceFile *) [self
  // memberWithDisplayName:[classDefinition sourceFileName]];
...
- (void)addFramework:(XCFrameworkDefinition *)frameworkDefinition {
  if (([self memberWithDisplayName:[frameworkDefinition fileName]]) == nil) {
    ...
    if ([frameworkDefinition copyToDestination]) {
      fileReference =
          [self makeFileReferenceWithPath:[frameworkDefinition fileName]
                                     name:nil
                                     type:Framework
                               sourceTree:frameworkDefinition.sourceTree];
      BOOL copyFramework = NO;
      if ([frameworkDefinition fileOperationType] ==
          XCFileOperationTypeOverwrite) {
        copyFramework = YES;
      } else if ([frameworkDefinition fileOperationType] ==
                 XCFileOperationTypeAcceptExisting) {
        NSString *frameworkName =
            [[frameworkDefinition filePath] lastPathComponent];
        if (![_fileOperationQueue
                            fileWithName:frameworkName
      ...
      if (copyFramework) {
        [_fileOperationQueue
            queueFrameworkWithFilePath:[frameworkDefinition filePath]
      ...
      fileReference =
          [self makeFileReferenceWithPath:path
                                     name:name
                                     type:Framework
                               sourceTree:frameworkDefinition.sourceTree];
    }
    NSString *frameworkKey =
        [[XCKeyBuilder forItemNamed:[frameworkDefinition fileName]] build];
...
- (XCSourceFile *)addFramework:(XCFrameworkDefinition *)frameworkDefinition
                     toTargets:(NSArray *)targets {
  [self addFramework:frameworkDefinition];
  XCSourceFile *frameworkSourceRef = (XCSourceFile *)[self
      memberWithDisplayName:[frameworkDefinition fileName]];
  [self addSourceFile:frameworkSourceRef toTargets:targets];
  return frameworkSourceRef;
}

- (void)removeFramework:(XCFrameworkDefinition *)frameworkDefinition
            fromTargets:(NSArray *)targets {
...
- (void)addFolderReference:(NSString *)sourceFolder {
  NSDictionary *folderReferenceDictionary =
      [self makeFileReferenceWithPath:sourceFolder
                                 name:[sourceFolder lastPathComponent]
                                 type:Folder
                           sourceTree:SourceTreeGroup];
  NSString *folderReferenceKey =
      [[XCKeyBuilder forItemNamed:[sourceFolder lastPathComponent]] build];
  [self addMemberWithKey:folderReferenceKey];
  [_project objects][folderReferenceKey] = folderReferenceDictionary;
  [_project objects][_key] = [self asDictionary];
}

- (void)addFileReference:(NSString *)filePath
                withType:(XcodeSourceFileType)type {
  NSDictionary *folderReferenceDictionary =
      [self makeFileReferenceWithPath:filePath
                                 name:[filePath lastPathComponent]
                                 type:type
                           sourceTree:SourceTreeGroup];
  NSString *folderReferenceKey =
      [[XCKeyBuilder forItemNamed:[filePath lastPathComponent]] build];
  [self addMemberWithKey:folderReferenceKey];
  [_project objects][folderReferenceKey] = folderReferenceDictionary;
  [_project objects][_key] = [self asDictionary];
}
...
- (XCGroup *)addGroupWithPath:(NSString *)path {
  ...
  for (id<XcodeGroupMember> groupMember in members) {
    if ([groupMember groupMemberType] == PBXGroupType ||
        [groupMember groupMemberType] == PBXVariantGroupType) {

      if ([[[groupMember pathRelativeToProjectRoot] lastPathComponent]
  ...
  XCGroup *group = [[XCGroup alloc] initWithProject:_project
                                                key:groupKey
                                              alias:nil
                                               path:path
                                           children:nil];
  ...
  [_fileOperationQueue queueDirectory:path
                          inDirectory:[self pathRelativeToProjectRoot]];
...
- (XCGroup *)addGroupWithAlias:(NSString *)alias
                     groupType:(XcodeMemberType)type {
  ...
  for (id<XcodeGroupMember> groupMember in members) {
    if ([groupMember groupMemberType] == PBXGroupType ||
      ...
      if ([[groupMember displayName] isEqualToString:alias] ||
          [[groupMember key] isEqualToString:groupKey]) {
        return nil;
      }
    }
...
- (void)addXib:(XCXibDefinition *)xibDefinition toTargets:(NSArray *)targets {
  ...
  XCSourceFile *sourceFile =
      [_project fileWithName:[xibDefinition xibFileName]];
  [self addSourceFile:sourceFile toTargets:targets];
}

// adds an xcodeproj as a subproject of the current project.
- (void)addSubProject:(XCSubProjectDefinition *)projectDefinition {
  // set up path to the xcodeproj file as Xcode sees it - path to top level of
  // project + _group path if any
  [projectDefinition initFullProjectPath:_project.filePath
                               groupPath:[self pathRelativeToParent]];

  // create PBXFileReference for xcodeproj file and add to PBXGroup for the
  // current _group
...
// adds an xcodeproj as a subproject of the current project, and also adds all
// build products except for test bundle(s)
// to targets.
- (void)addSubProject:(XCSubProjectDefinition *)projectDefinition
            toTargets:(NSArray *)targets {
  [self addSubProject:projectDefinition];

  // add subproject's build products to targets (does not add the subproject's
  // test bundle)
  NSArray *buildProductFiles =
      [_project buildProductsForTargets:[projectDefinition projectKey]];
  for (XCSourceFile *file in buildProductFiles) {
    [self addSourceFile:file toTargets:targets];
  }
  // add main target of subproject as target dependency to main target of
  // project
...
- (void)removeSubProject:(XCSubProjectDefinition *)projectDefinition {
  ...
  [_project removeFromProjectReferences:xcodeprojKey
                       forProductsGroup:productsGroupKey];
...
- (void)removeSubProject:(XCSubProjectDefinition *)projectDefinition
  ...
  // Remove PBXBundleFile entries and corresponding inclusion in
  // PBXFrameworksBuildPhase and PBXResourcesBuidPhase
  NSString *productsGroupKey = [_project productsGroupKeyForKey:xcodeprojKey];
  [self removeProductsGroupFromProject:productsGroupKey];

  // Remove the PBXContainerItemProxy for this xcodeproj with proxyType 1
  NSString *containerItemProxyKey = [_project
      containerItemProxyKeyForName:[projectDefinition pathRelativeToProjectRoot]
                         proxyType:@"1"];
...
- (NSString *)pathRelativeToProjectRoot {
  if (_pathRelativeToProjectRoot == nil) {
    ...
    _pathRelativeToProjectRoot =
        [[fullPath stringByAppendingPathComponent:_pathRelativeToParent] copy];
  }
...
- (void)becomeBuildFile {
  if (![self isBuildFile]) {
    if ([self canBecomeBuildFile]) {
      ...
      sourceBuildFile[@"isa"] =
          [NSString xce_stringFromMemberType:PBXBuildFileType];
      sourceBuildFile[@"fileRef"] = _key;
      NSString *buildFileKey = [[XCKeyBuilder
          forItemNamed:[self.displayName stringByAppendingString:@".buildFile"]]
          build];
...
- (NSString *)description {
  return [NSString stringWithFormat:@"Group: displayName = %@, key=%@",
                                    [self displayName], _key];
}
...
- (void)makeGroupMemberWithName:(NSString *)name
  ...
  if ((currentSourceFile) == nil) {
    ...
    filePath = [[currentSourceFile pathRelativeToProjectRoot]
  ...
  if (fileOperationStyle == XCFileOperationTypeOverwrite) {
    ...
    [_fileOperationQueue fileWithName:name existsInProjectDirectory:filePath];
             ...
             ![_fileOperationQueue fileWithName:name
                       existsInProjectDirectory:filePath]) {
    writeFile = YES;
  }
  if (writeFile) {
    if (![self fileTypeRequiresSpecialManagement:type]) {
      if ([contents isKindOfClass:[NSString class]]) {
        [_fileOperationQueue queueTextFile:name
                               inDirectory:filePath
                              withContents:contents];
      } else {
        [_fileOperationQueue queueDataFile:name
                               inDirectory:filePath
                              withContents:contents];
      }
    } else {
...
- (void)writeSpecialGroupMemberWithName:(NSString *)name
  ...
  if (type == XCDataModel) {
    ...
    [_fileOperationQueue
        queueTextFile:name
          inDirectory:filePath
         withContents:@"{\"info\" : {\"version\" : 1,\"author\" : \"xcode\"}}"];
  }
}
...
// creates PBXFileReference and adds to _group if not already there;  returns
// key for file reference.  Locates
// member via path rather than name, because that is how subprojects are stored
// by Xcode
- (void)makeGroupMemberWithName:(NSString *)name
                           path:(NSString *)path
                           type:(XcodeSourceFileType)type
...
- (NSString *)makeProductsGroup:(XCSubProjectDefinition *)xcodeprojDefinition {
  ...
  NSString *productKey = [[XCKeyBuilder
      forItemNamed:[NSString stringWithFormat:@"%@-Products", uniquer]] build];
  XCGroup *productsGroup = [XCGroup groupWithProject:_project
                                                 key:productKey
                                               alias:@"Products"
                                                path:nil
                                            children:children];
...
// makes a new Products _group (by calling the method above), makes a new
// projectReferences array for it and
// then adds it to the PBXProject object
- (void)addProductsGroupToProject:
...
// removes the given key from the files arrays of the given section, if found
// (intended to be used with
// PBXFrameworksBuildPhase and PBXResourcesBuildPhase)
// they are not required because we are currently not adding these entries;
// Xcode is doing it for us. The existing
// code for adding to a target doesn't do it, and I didn't add it since Xcode
// will take care of it for me and I was
// avoiding modifying existing code as much as possible)
- (void)removeBuildPhaseFileKey:(NSString *)key
                        forType:(XcodeMemberType)memberType {
  NSArray *buildPhases = [_project keysForProjectObjectsOfType:memberType
                                                withIdentifier:nil
                                                     singleton:NO
                                                      required:NO];
  for (NSString *buildPhaseKey in buildPhases) {
    NSDictionary *buildPhaseDict =
        [[_project objects] valueForKey:buildPhaseKey];
...
// removes entries from PBXBuildFiles, PBXFrameworksBuildPhase and
...
- (void)removeProductsGroupFromProject:(NSString *)key {
  ...
  for (NSString *childKey in [productsGroup valueForKey:@"children"]) {
    NSArray *buildFileKeys =
        [_project keysForProjectObjectsOfType:PBXBuildFileType
    ...
    if ([buildFileKeys count] == 1) {
      ...
      [self removeBuildPhaseFileKey:buildFileKey
                            forType:PBXFrameworksBuildPhaseType];
      [self removeBuildPhaseFileKey:buildFileKey
                            forType:PBXResourcesBuildPhaseType];
    }
  }
}
